...
let s:g.p={
            ...
            \      "fltstr": "Converted float to string: %s",
            \      "numstr": "Converted number to string: %s",
            \     "lsthash": "Unable to use list as a dictionary key",
            \     "dcthash": "Unable to use dictionary as a dictionary key",
            \    "nullhash": "Unable to use empty value as a dictionary key",
            \     "alundef": "Found undefined alias “%s”",
            \       "dupan": "Found duplicating anchor “%s”",
            ...
            \                "content, but found %s",
            \   "emptyflow": "While parsing a block node expected the node ".
            \                "content, but found %s",
            ...
            \     "nanname": "While scanning an %s expected ".
            \                "printable non-whitespace non-linebreak ".
            \                "character, but found “%s”",
            \      "nanend": "While scanning an %s expected ".
            \                "its end, but found “%s”",
            \   "ntagendgt": "While parsing a tag expected '>', but found “%s”",
            ...
            \   "indinplsc": "Unexpected indicator character in plain scalar: ".
            \                "“%s”",
            \    "abmcolon": "While scanning a plain scalar found unexpected ".
            \                "':'",
            \    "ntagbang": "While scanning a %s expected '!', but found “%s”",
            \     "ntaguri": "While scanning a %s expected URI, but found “%s”",
            \     "nuriesc": "While scanning a %s expected URI ".
            \                "escaped sequence of 2 hex digits, but found “%s”",
            \     "invchar": "Failed to decode character in position %u",
            \      "nprint": "Found non-printable character with code 0x%x ".
            \                "in position %u",
            ...
            \                'a single mapping item, but found %u items',
            \     "unundef": "Could not determine a constructor for the tag %s",
...
function s:F.main.destruct()
    for Cdf in s:g.load.classdeletes
        call call(Cdf, [], {})
        " TODO report a bug:
        " if unlet is replaced with delfunction, then E685 error is echoed
        unlet Cdf
    endfor
    unlet s:g
    unlet s:F
...
"{{{3 load.Loader.__init__ :: (stream) -> _
...
"{{{3 load.Loader.__raise
function s:F.load.Loader.__raise(selfname, type, msgid, ...)
    return s:F.main.eerror(a:selfname, a:type, 1, [a:msgid]+a:000)
endfunction
"{{{3 load.Loader.__warn
function s:F.load.Loader.__warn(selfname, type, msgid, ...)
    return s:F.main.eerror(a:selfname, a:type, [a:msgid]+a:000)
endfunction
"{{{3 load.Reader
    ...
    while len(self.buffer)<a:length
        ...
        if type(self.raw_decode)==2
            ...
            if converted<0
                ...
                call self.__raise(selfname, "reader", "invchar", position)
            endif
        else
...
function s:F.load.Reader.check_printable(data)
    ...
    for ch in a:data
        if ch!~#'^'.s:g.yaml.printchar.'\=$'
            ...
            call self.__raise(selfname, "reader", "nprint", char2nr(ch),
                        \     position)
        endif
        let index+=1
    endfor
...
function s:F.load.Parser.parse_document_start()
    ...
    if !self.check_token("StreamEndToken")
        ...
        if !self.check_token("DocumentStartToken")
            call self.__raise(selfname, "parser", "ndocstart",
                        \     self.peek_token().__class__)
        endif
        let token=self.get_token()
...
function s:F.load.Parser.parse_node(block, indentless_sequence)
    ...
    if self.check_token("AliasToken")
        ...
        if tag!=[]
            ...
            if handle!=#""
                if !has_key(self.tag_handles, handle)
                    call self.__raise(selfname, "parser", "ukntag", handle)
                endif
        ...
        if a:indentless_sequence && self.check_token("BlockEntryToken")
            ...
            elseif anchor!=#"" || tag!=#""
                ...
                let token=self.peek_token()
                if a:block
                    call self.__raise(selfname, "parser", "emptblock",
                                \     token.__class__)
                else
                    call self.__raise(selfname, "parser", "emptyflow",
                                \     token.__class__)
                endif
            endif
        endif
...
function s:F.load.Parser.parse_block_sequence_entry()
    ...
    if !self.check_token("BlockEndToken")
        let token=self.peek_token()
        call self.__raise(selfname, "parser", "nblkend", token.__class__)
    endif
    let token=self.get_token()
    let event=s:F.plug.oop.getinstance("SequenceEndEvent", token.start_mark,
                \                      token.end_mark)
...
function s:F.load.Parser.parse_block_mapping_key()
    ...
    if !self.check_token("BlockEndToken")
        let token=self.peek_token()
        call self.__raise(selfname, "parser", "nblkmend", token.__class__)
    endif
    let token=self.get_token()
    ...
    if !self.check_token("FlowSequenceEndToken")
        if !first
            if self.check_token("FlowEntryToken")
                ...
                let token=self.peek_token()
                call self.__raise(selfname, "parser", "nflwseq",
                            \     token.__class__)
            endif
            if self.check_token("KeyToken")
                let token=self.peek_token()
    ...
    if !self.check_token("FlowMappingEndToken")
        if !first
            if self.check_token("FlowEntryToken")
                ...
                let token=self.peek_token()
                call self.__raise(selfname, "parser", "nflwmap",
                            \     token.__class__)
            endif
        endif
        if self.check_token("KeyToken")
...
function s:F.load.Scanner.scan_anchor(tokenclass)
    ...
    endwhile
    if !length
        call self.__raise(selfname, "scanner", "nanname", name, ch)
    endif
    let value=self.prefix(length)
    call self.forward(length)
    let ch=self.peek()
    if ch!~#self.ANEND
        call self.__raise(selfname, "scanner", "nanend", name, ch)
    endif
...
function s:F.load.Scanner.scan_block_scalar_ignored_line(start_mark)
    ...
    if ch!~#'^['.s:g.yaml.linebreak.']\=$'
        call self.__raise(selfname, "scanner", "nignln", ch)
    endif
...
function s:F.load.Scanner.scan_block_scalar_indicators(start_mark)
    ...
    if ch=~#'^\d$'
        let increment=ch+0
        if increment==0
            call self.__raise(selfname, "scanner", "nullindnt")
        endif
        call self.forward()
        if chomping==-1 && (ch==#'+' || ch==#'-')
            let chomping=(ch==#'+')
    ...
    if ch!~#self.SEPREGEX
        call self.__raise(selfname, "scanner", "nblkind", ch)
    endif
...
function s:F.load.Scanner.scan_flow_scalar_breaks(double, start_mark)
    ...
    while 1
        ...
        if (prefix==#'---' || prefix==#'...') && self.peek(3)=~#self.SEPREGEX
            call self.__raise(selfname, "scanner", "docsepqs")
        endif
        while self.peek()=~#'^['.s:g.yaml.whitespace.']$'
            call self.forward()
        endwhile
    ...
    while 1
        ...
        elseif ch==#'\'
            ...
            elseif has_key(self.ESCAPE_CODES, ch)
                ...
                while k<length
                    if self.peek(k)!~#'^\x$'
                        call self.__raise(selfname, "scanner", "ndqs", length,
                                    \     self.peek())
                    endif
                    let code=str2nr(self.prefix(length), 16)
                    if code==0
                        call self.__warn(selfname, "notimp", "strnull")
                    else
            ...
            elseif ch==#'0'
                ...
                call self.__raise(selfname, "scanner", "uknesc", ch)
            endif
        else
            return chunks
        endif
...
function s:F.load.Scanner.scan_flow_scalar_spaces(double, start_mark)
    ...
    let ch=self.peek()
    if ch==#''
        call self.__raise(selfname, "scanner", "qseos")
    elseif ch=~#'^['.(s:g.yaml.linebreak).']$'
        let line_break=self.scan_line_break()
...
function s:F.load.Scanner.scan_tag()
    ...
    if ch==#'<'
        ...
        if self.peek()!=#'>'
            call self.__raise(selfname, "scanner", "ntagendgt", self.peek())
        endif
        call self.forward()
    else
        let length=1
    ...
    let ch=self.peek()
    if ch!~#self.SEPREGEX
        call self.__raise(selfname, "ntagend", ch)
    endif
...
function s:F.load.Scanner.scan_plain()
    ...
    while 1
        ...
        if (self.flow_level && ch==#s:g.yaml.mappingvalue &&
                    ...
                    \                           (s:g.yaml.linebreak).']\=$')
            " XXX
            call self.__warn(selfname, "scanner", "ambcolon")
        endif
        if length==0
            break
        endif
...
function s:F.load.Scanner.scan_uri_escapes(name, start_mark)
    ...
    while self.peek()=='%'
        ...
        while k<2
            if self.peek(k)!~#'^\x$'
                call self.__raise(selfname, "scanner", "nuriesc", a:name,
                            \     self.peek(k))
            endif
            let k+=1
        endwhile
...
function s:F.load.Scanner.scan_tag_uri(name, start_mark)
    ...
    if chunks==[]
        call self.__raise(selfname, "scanner", "ntaguri", a:name, ch)
    endif
    return join(chunks, "")
endfunction
...
function s:F.load.Scanner.scan_tag_handle(name, start_mark)
    ...
    let ch=self.peek()
    if ch!=#'!'
        call self.__raise(selfname, "scanner", "ntagbang", a:name, ch)
    endif
    let length=1
    let ch=self.peek(length)
...
function s:F.load.Scanner.scan_tag_directive_prefix(start_mark)
    ...
    let ch=self.peek()
    if ch!~#self.SEPREGEX
        call self.__raise(selfname, "scanner", "nTAGesep", ch)
    endif
endfunction
...
function s:F.load.Scanner.scan_tag_directive_handle(start_mark)
    ...
    let ch=self.peek()
    if ch!~#'^['.s:g.yaml.whitespace.']$'
        call self.__raise(selfname, "scanner", "nTAGsep", ch)
    endif
    return value
endfunction
...
function s:F.load.Scanner.scan_yaml_directive_number(start_mark)
    ...
    let ch=self.peek()
    if ch!~#'^\d$'
        call self.__raise(selfname, "scanner", "nYAMLd")
    endif
    let length=0
...
function s:F.load.Scanner.scan_yaml_directive_value(start_mark)
    ...
    if self.peek()!=#'.'
        call self.__raise(selfname, "scanner", "nYAMLdsep", self.peek())
    endif
    call self.forward()
    let minor=self.scan_yaml_directive_number(a:start_mark)
    if self.peek()!~#self.SEPREGEX
        call self.__raise(selfname, "scanner", "nYAMLws", self.peek())
    endif
    return [major, minor]
endfunction
...
function s:F.load.Scanner.scan_directive_ignored_line(start_mark)
    ...
    let ch=self.peek()
    if ch!~#'^['.s:g.yaml.linebreak.']\=$'
        call self.__raise(selfname, 'ndirlend', ch)
    endif
    call self.scan_line_break()
endfunction
...
function s:F.load.Scanner.scan_directive_name(start_mark)
    ...
    endwhile
    if !length
        call self.__raise(selfname, "scanner", "ndirname", ch)
    endif
    let value=self.prefix(length)
    call self.forward(length)
    let ch=self.peek()
    if ch!~#self.SEPREGEX
        call self.__raise(selfname, "scanner", "ndirend", ch)
    endif
    return value
endfunction
...
function s:F.load.Scanner.fetch_block_entry()
    ...
    if !self.flow_level
        if !self.allow_simple_key
            call self.__raise(selfname, "scanner", "seqnall")
        endif
...
function s:F.load.Scanner.fetch_more_tokens()
    ...
    call self.__raise(selfname, "scanner", "nottkstrt", ch)
endfunction
"{{{3 load.BaseResolver
...
function s:F.load.Composer.compose_mapping_node(anchor)
    ...
    while !self.check_event("MappingEndEvent")
        ...
        " if has_key(node.value, item_key)
            " call self.__warn(selfname, "composer", "dupkey", item_key)
        " endif
...
function s:F.load.Composer.compose_node(parent, index)
    ...
    if self.check_event("AliasEvent")
        ...
        if !has_key(self.anchors, anchor)
            call self.__raise(selfname, "composer", "alundef", anchor)
        endif
    endif
    ...
    if has_key(event, "anchor")
        let anchor=event.anchor
        if has_key(self.anchors, anchor)
            call self.__warn(selfname, "composer", "dupan", anchor)
        endif
    else
        let anchor=""
    endif
    ...
    if !exists("l:node")
        call self.__raise(selfname, "_internal", "ndef", "node")
    endif
    return node
endfunction
...
function s:F.load.Composer.get_single_node()
    ...
    if !self.check_event("StreamEndEvent")
        let event=self.get_event
        call self.__warn(selfname, "composer", "notsingle")
    endif
    call self.get_event()
    return document
endfunction
...
function s:F.load.BaseConstructor.__init__(super)
    ...
    let self.state_generators=[]
    let self.deep_construct=0
endfunction
...
function s:F.load.BaseConstructor.construct_scalar(node, ...)
    ...
    if a:node.__class__!=#"ScalarNode"
        call self.__raise(selfname, "constructor", "notsc", a:node.__class__)
    endif
    return a:node.value
endfunction
...
function s:F.load.BaseConstructor.construct_sequence(node, ...)
    ...
    let deep=0
    if a:000!=[] && a:000[0]
        let old_deep=self.deep_construct
        let self.deep_construct=1
        let deep=1
    endif
    if a:node.__class__!=#"SequenceNode"
        call self.__raise(selfname, "constructor", "notseq", a:node.__class__)
    endif
    let sequence=[]
    for value_node in a:node.value
        call add(sequence, self.construct_object(value_node, deep))
    endfor
    return sequence
endfunction
...
function s:F.load.BaseConstructor.construct_mapping(node, ...)
    ...
    if a:node.__class__!=#"MappingNode"
        call self.__raise(selfname, "constructor", "notmap", a:node.__class__)
    endif
    let mapping={}
    for [key_node, value_node] in a:node.value
        let key=self.construct_object(key_node, deep)
        let tkey=type(key)
        if tkey!=type("")
            if tkey==type(0)
                let key=string(key)
                call self.__warn(selfname, "notimp", "numstr", key)
            elseif tkey==type(0.0)
                let tmp=string(key)
                unlet key
                let key=tmp
                call self.__warn(selfname, "notimp", "fltstr", key)
            elseif tkey==type([])
                call self.__raise(selfname, "notimp", "lsthash")
            elseif tkey==type({})
                call self.__raise(selfname, "notimp", "dcthash")
            endif
        elseif key==#""
            call self.__raise(selfname, "notimp", "nullhash")
        endif
        let value=self.construct_object(value_node, deep)
...
function s:F.load.BaseConstructor.construct_object(node, ...)
    ...
    elseif has_key(self.recursive_objects, a:node.id)
        call self.__raise(selfname, "constructor", "invrec")
    endif
    " XXX self.recursive_objects[node]=None
    ...
    if type(data)==type({}) && get(data, "type", "")==#"Generator"
        let generator=data
        unlet data
...
function s:F.load.BaseConstructor.construct_document(node)
    ...
    while self.state_generators!=[]
        let state_generators=self.state_generators
        let self.state_generators=[]
        " XXX Непонятно, что этот код должен делать
        for generator in state_generators
            call generator.drop()
        endfor
        let self.constructed_objects={}
        let self.recursive_objects={}
        let self.deep_costruct=0
    endwhile
    return data
endfunction
...
function s:F.load.SafeConstructor.flatten_mapping(node)
        ...
        if key_node.tag==#'tag:yaml.org,2002:merge'
            ...
            elseif value_node.__class__==#"SequenceNode"
                ...
                for value in submerge
                    call add(merge, value)
                endfor
            else
                call self.__raise(selfname, "constructor", "nmapseq",
                            \     value_node.__class__)
            endif
    ...
    endwhile
    if merge!=[]
        let a:node.value=extend(merge, a:node.value)
    endif
endfunction
...
function s:F.load.SafeConstructor.construct_yaml_omap(node)
    ...
    let omap=[]
    if node.__class__!=#"SequenceNode"
        call self.__raise(selfname, "constructor", "nseqomap", node.__class__)
    endif
    for subnode in node.value
        if node.__class__!=#"MappingNode"
            call self.__raise(selfname, "constructor", "nmapomap",
                        \     node.__class__)
        endif
        if len(subnode.value)!=1
            call self.__raise(selfname, "constructor", "nmlenomap",
                        \     len(node.value))
        endif
...
function s:F.load.SafeConstructor.construct_yaml_pairs(node)
    ...
    let pairs=[]
    if node.__class__!=#"SequenceNode"
        call self.__raise(selfname, "constructor", "nseqpr", node.__class__)
    endif
    for subnode in node.value
        if node.__class__!=#"MappingNode"
            call self.__raise(selfname, "constructor", "nmappr",
                        \     node.__class__)
        endif
        if len(subnode.value)!=1
            call self.__raise(selfname, "constructor", "nmlenpr",
                        \     len(node.value))
        endif
        let [key_node, value_node]=subnode.value[0]
        let key=self.construct_object(key_node)
...
function s:F.load.SafeConstructor.construct_undefined(node)
    ...
    call self.__raise(selfname, "constructor", "unundef", a:node.tag)
endfunction
"{{{3 load.Constructor
...
function s:F.load.Constructor.construct_vim_function(node)
    ...
    if value!~#'^\d*$'
        if value[0:1]==#'s:'
            call self.__raise(selfname, "constructor", "fscript", value)
        endif
        if exists('*'.value)
            return function(value)
        else
            call self.__raise(selfname, "constructor", "fundef", value)
        endif
    else
        call self.__raise(selfname, "notimp", "fnum", str2nr(value))
    endif
endfunction
