...
if !exists('s:_pluginloaded')
    execute frawor#Setup('1.0', {'@oop': '1.0',
                \         '@/resources': '0.0',
                \            '@/base64': '0.0',}, 0)
    call map(['comm', 'yaml', 'load', 'dump'], 'extend(s:F, {v:val : {}})')
    lockvar 1 s:F
    finish
...
let s:load.Scanner={
        ...
        \}
if has('float')
    let s:load.SafeConstructor={
                \'inf_value': 1.0e300,
            \}
    " pow(...) is shorter
    while s:load.SafeConstructor.inf_value!=
                \pow(s:load.SafeConstructor.inf_value, 2)
        let s:load.SafeConstructor.inf_value=
                    \pow(s:load.SafeConstructor.inf_value, 2)
    endwhile
    let s:load.SafeConstructor.nan_value=
                \-(s:load.SafeConstructor.inf_value)/
                \ (s:load.SafeConstructor.inf_value)
endif
let s:load.DirectiveToken={'id': '<directive>',}
let s:load.DocumentStartToken={'id': '<document start>',}
let s:load.DocumentEndToken={'id': '<document end>',}
let s:load.StreamStartToken={'id': '<stream start>',}
let s:load.StreamEndToken={'id': '<stream end>',}
"{{{3 Ошибки
...
function s:F.load.Reader.check_printable(data)
    ...
    let position=self.index+(len(self.buffer)-self.pointer)
    for ch in a:data
        if !empty(ch) && !s:F.comm.isprintchar(ch) && !(position==0 &&
                    \                                   ch is# "\uFEFF")
...
function s:F.load.Scanner.fetch_more_tokens()
    ...
    elseif has_key(s:load.scanner.chtofname, ch)
        return self[s:load.scanner.chtofname[ch]]()
...
let s:load.safeconstructor.timestampregex=
            \'^\d\d\d\d'.
            \'-\d\d\='.
            \'-\d\d\='.
            ...
            \                                  '\%(:\(\d\d\)\)\=\)\)\=\)\=$'
let s:load.safeconstructor.tsstartregex=
            \'^\(\d\d\d\d\)'.
            \'-\(\d\d\=\)'.
            \'-\(\d\d\=\)'
function s:F.load.SafeConstructor.construct_yaml_timestamp(node)
    let value=self.construct_scalar(a:node)
    let matches=matchlist(value, s:load.safeconstructor.timestampregex)
    let matches0=matchlist(value, s:load.safeconstructor.tsstartregex)
...
"{{{3 load.setclass
function s:F.load.setclass(name, ...)
    call s:_f.pyoop.class(a:name, get(s:F.load, a:name, {}),
                \                 get(s:load, a:name, {}),
                \                 a:000)
endfunction
"{{{3 Создание классов
...
function s:F.dump.choose_scalar_style(scalar, opts, iskey)
    ...
    while i<slen
        ...
        if !s:F.comm.isprintchar(char) || (char is# "\u2029" ||
                    \                      char is# "\u2028")
            let plain=0
            let single=0
            let literal=0
...
function s:F.dump.dumplst(obj, r, dumped, opts)
    ...
    for d.Item in a:obj
        ...
        call s:F.dump.dump(d.Item, a:r, a:dumped, a:opts, islocked)
        unlet d.Item
        if get(a:opts, 'all_flow', 0)
            let a:r[-1].=','
        endif
...
function s:F.dump.dumpdct(obj, r, dumped, opts)
    ...
    for key in keylist
        ...
        let a:r[-1].=':'
        call s:F.dump.dump(d.Value, a:r, a:dumped, a:opts, 0)
        unlet d.Value
        if get(a:opts, 'all_flow', 0)
            let a:r[-1].=','
        endif
...
"{{{3 dump.dump
"{{{4 s:dumptypes
let s:dumptypes=[s:F.dump.dumpnum, s:F.dump.dumpstr, s:F.dump.dumpfun,
...
"{{{1 yaml resource
call s:_f.postresource('yaml', {'loads': s:F.load.loads,
            \                'load_all': s:F.load.load_all,
            \                   'dumps': s:F.dump.dumps,})
"{{{1
